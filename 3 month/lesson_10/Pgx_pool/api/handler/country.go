...
func (h *Handler) CreateCountry(c *gin.Context) {
	...
	resp, err := h.strg.Country().Create(c, country)
	if err != nil {
...
func (h *Handler) CountryGetById(c *gin.Context) {
	id := c.Param("id")
	resp, err := h.strg.Country().GetById(c, models.CountryPrimaryKey{Id: id})
	if err != nil {
...
func (h *Handler) CountryGetList(c *gin.Context) {
	...
	resp, err := h.strg.Country().GetList(c, models.GetListCountryRequest{Offset: offset, Limit: limit})
	if err != nil {
		handleResponse(c, 500, "Country does not exist: "+err.Error())
		return
	}
...
func (h *Handler) CountryUpdate(c *gin.Context) {
	...
	id := c.Param("id")
	country.Id = id
	resp, err := h.strg.Country().Update(c, country)
	if err != nil {
...
func (h *Handler) CountryDelete(c *gin.Context) {
	id := c.Param("id")
	_, err := h.strg.Country().Delete(c, models.CountryPrimaryKey{Id: id})
	if err != nil {
...
func (h *Handler) UploadCountry(c *gin.Context) {
	...
	err = h.strg.Country().Upload(c, models.UploadCountry{Base: "./api/upload", File: *multFile})
