...
func (h *Handler) CreateCity(c *gin.Context) {
	...
	resp, err := h.strg.City().Create(c, city)
	if err != nil {
...
func (h *Handler) CityGetById(c *gin.Context) {
	...
	resp, err := h.strg.City().GetById(c, models.CityPrimaryKey{Id: id})
	if err != nil {
...
func (h *Handler) CityGetList(c *gin.Context) {
	...
	resp, err := h.strg.City().GetList(c, models.GetListCityRequest{
		Offset: int(offset),
		Limit:  int(limit),
	})
	if err != nil {
...
func (h *Handler) CityUpdate(c *gin.Context) {
	...
	id := c.Param("id")
	city.Id = id
	resp, err := h.strg.City().Update(c, city)
	if err != nil {
...
func (h *Handler) CityDelete(c *gin.Context) {
	...
	_, err := h.strg.City().Delete(c, models.CityPrimaryKey{Id: id})
	if err != nil {
...
func (h *Handler) UploadCity(c *gin.Context) {
	multFile, err := c.FormFile("file")
	if err != nil {
		handleResponse(c, 404, "file not")
		return
	}
	err = h.strg.City().Upload(c, models.UploadCity{Base: "./api/upload", File: *multFile})
